import {Address, MessageID, WorkerPath} from "./Worker";
...
export class ActorSystem {
    ...
    constructor(public address: string, public opts: string) {
        ...
        const mailboxForwarderSubscription = mailbox$.pipe(
     ...
     */
    public actorOf(workerPath: string|Function, name = uuid(), parent = '/system'): ActorRef {
        const address = [parent, name].join('/');
        const w = typeof workerPath === 'string' ? new Worker(workerPath) : workerPath();
        const m: Message = {
